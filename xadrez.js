	...
	function cria_array(){
		...
		for(x=1;x<=8;x++){
			...
			for(y=1;y<=8;y++){
				...
				peca[x][y] = new Array();
				peca[x][y]['peca'] = false; 		//defino como falso para as que ṇo come̤am com pe̤a ficarem nulas
				peca[x][y]['cor'] = false;			//defino como falso para as que ṇo come̤am com pe̤a ficarem nulas
...
function seleciona(x,y){
		...
		if((movimenta['selecionada']['x']==0 || peca[x][y]['cor'] == movimenta['selecionada']['cor']) && peca[x][y]['cor']==vez){
			...
			if(peca[x][y]['peca']){ //se tiver uma peca nessa posi̤̣o
			...
			if(peca[x][y]['cor'] != movimenta['selecionada']['cor']){
				...
				if(peca[x][y]['peca']){  //se tiver alguma peca nessa posi̤̣o
				...
				document.getElementById("t"+x+""+y).className = movimenta['selecionada']['peca']+"-"+movimenta['selecionada']['cor']; //destino recebe a imagem da pe̤a selecinada
